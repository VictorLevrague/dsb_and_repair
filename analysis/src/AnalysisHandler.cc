...
void AnalysisHandler::SetPathToOutputs(std::string path) // Ajout de la fonction (Mathieu)
{
    fPathToOutputs = path;
    fScanDamage->SetPathToOutputs(path);
}
...
void AnalysisHandler::GetAllDamageAndScanSB()
{
    ...
    //---> print DSB for RITRACKS results
...
void AnalysisHandler::GiveMeSBs()
{
    ...
    std::string outName = fPathToOutputs + "/" + ParametersParser::Instance()->GetOutputName();
    std::fstream file;
	file.open(outName.c_str(), std::ios_base::out);
    double norm = 1.0;
    std::string normunit = "";
...
void AnalysisHandler::CreateSDD(std::string filename)
{
    ...
    std::string outName = fPathToOutputs + "/" + filename;

	ofile.open(outName.c_str(), std::ios_base::out);
    if (ofile.is_open()) {
        // Create header
        ofile
            <<"SDD version, SDDv1.0;\n"
