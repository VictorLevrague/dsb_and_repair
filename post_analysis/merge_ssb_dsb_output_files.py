...
def merge_ssb_dsb_files():
    analysis_output_folder = f"{PATH}/post_analysis/analysis_output/{ION_ENERGY}/Irradiation{IRRADIATION_ID}"
...
def write_output_file(damage_type, output_folder):
    with open(f"{output_folder}/List_{damage_type}.txt", "w") as output_file:
        ...
        for copy_nb in range(0, NB_SIMULATIONS):
            damage_file_name = f"{PATH}/output/output{ION_ENERGY}/outputIrradiation{IRRADIATION_ID}/Copy{copy_nb}/List_{damage_type}.txt"
