...
#include "InformationKeeper.hh" //Mathieu
...
void ChemGeoImport::ParseChemInputFile(const G4String& fileName)
{
    ...
    while(std::getline(file, line) )
    {
        ...
        else if(firstItem=="_eventNum")
        {
            ...
            issLine >> eventNumber; //Mathieu
            ChemNtupleManager::Instance()->SetEventNumber(eventNumber); //Mathieu
        }
        ...
        {
            G4double voxelCopyNumber; //Mathieu
            issLine >> voxelCopyNumber; //Mathieu
            ChemNtupleManager::Instance()->SetVoxelCopyNumber(voxelCopyNumber); //Mathieu
        }
...
void ChemGeoImport::GetVoxelDefFilePathList()
{
    ...
    //G4fs::path thisP = InformationKeeper::Instance()->GetMyFolderName();
    std::string thisP = InformationKeeper::Instance()->GetMyFolderName();
    G4cout << "path = " << thisP << G4endl; // print thisP pour savoir si l'on est bien sur le bon repertoire (Mathieu)
