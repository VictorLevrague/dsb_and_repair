    ...
    const std::vector<G4Track*>* products)
{
                ...
                ) )
    {
        ...
        if (_dnaMolecule) {
            ...
            if(fReactif1==8 || fReactif2 ==8){

                dnaMol = 8;
            }
            else if(fReactif1==9 || fReactif2 ==9){
            ...
            G4int eventNumber = ChemNtupleManager::Instance()->GetEventNumber();
            G4int voxelCopyNumber = ChemNtupleManager::Instance()->GetVoxelCopyNumber();



            // Retrieve the molecule coordinates
            ChemNtupleManager::Instance()->FillNtupleDColumn(1, 0, strand);
            ChemNtupleManager::Instance()->FillNtupleDColumn(1, 1, copyNo);
            //ChemNtupleManager::Instance()->FillNtupleDColumn(1, 2, radical->GetPosition().getX()/nm);
            //ChemNtupleManager::Instance()->FillNtupleDColumn(1, 3, radical->GetPosition().getY()/nm);
            //ChemNtupleManager::Instance()->FillNtupleDColumn(1, 4, radical->GetPosition().getZ()/nm);
            ChemNtupleManager::Instance()->FillNtupleDColumn(1, 2, G4Scheduler::Instance()->GetGlobalTime()/ns );
            ChemNtupleManager::Instance()->FillNtupleDColumn(1, 3, dnaMol);
            ChemNtupleManager::Instance()->FillNtupleDColumn(1, 4, eventNumber);
            ChemNtupleManager::Instance()->FillNtupleDColumn(1, 5, voxelCopyNumber);
            ChemNtupleManager::Instance()->AddNtupleRow(1);
        } else {
