...
#include "G4PeriodicBoundaryBuilder.hh" // Importation du code G4PeriodicBoundaryBuilder.hh (Mathieu)
...
G4VPhysicalVolume * DetectorConstruction::ConstructFullCellNucleusGeo()
{
    ...
    // Reconstruction du LogicWorld en prenant en compte le pbc (Mathieu) 
    auto pbc = std::make_unique<G4PeriodicBoundaryBuilder>();
    fLogicWorldPBC = pbc->Construct(fLogicWorld);

    // then place cell nucleus in world
    new G4PVPlacement(0, G4ThreeVector(), logicNucleus, "nucleus_pl", fLogicWorldPBC, false, false); // Remplacemnt de fLogicWorld par fLogicWorldPBC
    ...
    time_t timeStart = InformationKeeper::Instance()->GetTimeStart();
    time_t time_current;
    time(&time_current);
    G4cout<<"temps post Parsing Voxels " << difftime(time_current,timeStart)/60 << " minutes" << G4endl;
    ...
    if (voxelMap.size() > 0) {
        ...
        for (auto it = voxelMap.begin(); it != voxelMap.end(); it++) {
            ...
            new G4PVPlacement(0, G4ThreeVector(posX,posY,posZ), 
                            it->second, "xx", fLogicWorldPBC, false, 0); // Remplacement de fLogicWorld par fLogicWorldPBC pour le placement des voxels fictifs (Mathieu)
        }
    ...
    time_t time_2;
    time(&time_2);
    G4cout<<"temps post CreateFullNucleus " << difftime(time_2,timeStart)/60 << " minutes" << G4endl;
    return fPhysWorld;
}
