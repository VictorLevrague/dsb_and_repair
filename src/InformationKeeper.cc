...
void InformationKeeper::WritePhysGeo(G4String pathToOutputs) // changement fPathToOutputs au lieu de CopyNumber (Mathieu)
{
    //Create a file containing geopath and Physlist in "FS build directory" for chemStage and anlysis

    //std::string fname = "imp.info";
    //std::string fname = "output/Copy"+CopyNumber+"/imp.info"; // modification de fname (Mathieu), remplacment de "output" par fPathToOutputs
    std::string fname = pathToOutputs + "/imp.info";
...
void InformationKeeper::SetFolderName(G4String path)
{
    // Creation des nouveau paths des fichiers d'outputs chem_input et phys_output (Mathieu)
    fPathToOutputs = path; //Mathieu
    if (path == ""){
        fPhysOutFolderName = "phys_output";
        fChemInputFolderName = "chem_input";
    }
    else {
        fPhysOutFolderName = path+"/phys_output";


        fChemInputFolderName = path+"/chem_input";
    }
