...
void PhysAnalysis::Book()
{
    ...
    //anManager->CreateNtupleDColumn("flagParentID"); //Mathieu
...
void PhysAnalysis::UpdateChemInputDataAndFillNtuple()
{
    ...
    for (auto const& binfo : fInfoInPhysStageVector) {
        ...
        // Ajout de l'ionization par GenericIon, fFlagProcess = 33 (Mathieu)
        ...
        // Si l'on est sur un phosphate ou un sucre alors on remplie le nutple (Mathieu)
        if(binfo.fVolumeName == 1      // d1
       || binfo.fVolumeName == 11  // p1
       || binfo.fVolumeName == 2   // d2
       || binfo.fVolumeName == 22  // p2
       ...
       || binfo.fVolumeName == 81) // p2_w
      {
        ...
        //analysisManager->FillNtupleDColumn(1, 1, binfo.fFlagParentID); //Mathieu
        analysisManager->FillNtupleDColumn(1, 1, binfo.fFlagProcess);
        ...
        analysisManager->FillNtupleDColumn(1, 2, binfo.fEdep);
        analysisManager->FillNtupleDColumn(1, 3, binfo.fEventNumber);
        analysisManager->FillNtupleDColumn(1, 4, binfo.fVolumeName);
        analysisManager->FillNtupleDColumn(1, 5, binfo.fCopyNumber);
        analysisManager->FillNtupleDColumn(1, 6, binfo.fLastMetVoxelCopyNum);
...
G4String PhysAnalysis::CreateChemInputFile(G4int eventNum, G4int volumeCopyNumber, const G4String &voxelName)
{
    ...
    sstream<<fOutputFolder<<"/event_"<<eventNum<<"_voxel_"<<volumeCopyNumber<<".dat"; // Mathieu suppression de "./"
    std::ofstream oFile;
