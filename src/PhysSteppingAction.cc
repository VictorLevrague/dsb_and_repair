...
// L'idée est de print sur le terminal les phenomenes de pbc (Mathieu)
    //G4int is_pbc_activated; // declaration de la variable is_pbc_activated (Mathieu)
    //is_pbc_activated = G4UIcommand::ConvertToInt(argv[4]); // is_pbc_activated = dernier argument (Mathieu) 

    //if (is_pbc_activated){ 
        //auto track = step->GetTrack();
        //auto userInf = track->GetUserInformation();
        //if (dynamic_cast<BoundaryInfo*>(userInf)->GetTrackBoundaryID() >= 1) 
        //{G4cout << "pbc number" << dynamic_cast<BoundaryInfo*>(userInf)->GetTrackBoundaryID() << G4endl;}
    //}
// Fin de la modification (Mathieu)
    if(fFlagVolume>0)
    {
        fEventAction->AddEdep(step->GetTotalEnergyDeposit()/eV);
    }
    if (fFlagProcess == 12 || fFlagProcess == 13 
        || fFlagProcess == 112 || fFlagProcess == 113 
        || fFlagProcess == 15 || fFlagProcess == 17
        || fFlagProcess == 18 || fFlagProcess == 20
        || fFlagProcess == 21 || fFlagProcess == 23
        || fFlagProcess == 24 || fFlagProcess == 26
        || fFlagProcess == 27 || fFlagProcess == 30
        || fFlagProcess == 31 || fFlagProcess == 33)
        {
                ...
                || fFlagVolume == 12) // thy_w
        {
            ...
            if(step->GetPostStepPoint()->GetPhysicalVolume() ) // avoid asking non existing information in postStep
            {
                ...
                G4int x_vox = G4int((x+8000+20)/40)*40; // Mathieu
                G4int y_vox = G4int((y+8000+20)/40)*40; // Mathieu
                G4int z_vox = G4int((z+8000+20)/40)*40; // Mathieu

                std::array<G4int,3> vox_pos = {x_vox,y_vox,z_vox}; //Mathieu
                auto pos = g_map_voxel.find(vox_pos); //Mathieu
                G4int vox_copyNumber = pos->second; //Mathieu
    #ifdef USE_MPI
                ...
                aInfo.fLastMetVoxelCopyNum = vox_copyNumber;//Mathieu
...
G4double PhysSteppingAction::SetupParticleFlag(const G4String& particleName)
{
    ...
    // ajout des flagParticle pour les ions d'interets (Mathieu)

    else if (particleName == "C12")   flagParticle = 12;
    else if (particleName == "O16")   flagParticle = 16;
    else if (particleName == "Si28")  flagParticle = 28;
    else if (particleName == "Ti48")  flagParticle = 48;
    else if (particleName == "Fe56")  flagParticle = 56;
...
G4double PhysSteppingAction::SetupProcessFlag(const G4String& processName)
{
    ...
    // ajout du flagProcess ionisation pour les genericIon (Mathieu)

    else if (processName=="GenericIon_G4DNAIonisation") flagProcess =33;
